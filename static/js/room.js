...
async function send(name, time) {
    ...
    if (ca != '') {
        socket.on('connect', function() {
            var x = socket.emit('update', {
                data: `{"time":"${ca}","name":"${name}","id":${window.id}}`
            });
...
socket.on("change_event", function(msg) {
	msg = JSON.parse(msg)
	if (msg["rid"] == window.id){
	window.event = msg['evn']
	document.getElementById('event').value = window.event 
...
function chevn(ent) {
    var ev = ["3x3", "2x2", "4x4", "5x5", "pyraminx", "skewb", "clock", "3bld", 'megaminx']
    window.event = ent
    ...
    if (window.event == "megaminx") {

        document.getElementById("sc").style.font_size = "1.5vw"
    }
}
